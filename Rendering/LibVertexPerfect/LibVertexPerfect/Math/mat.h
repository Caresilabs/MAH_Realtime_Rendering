...
namespace linalg {
	...
	public:
		 ...
		 */
		mat4( const T& _m11, const T& _m12, const T& _m13, const T& _m14,
		...
		//
		// rotation from Euler angles: roll (R_z), pitch (R_y), yaw (R_x)
		// R = R_z(a) * R_y(b) * R_x(c)
		//
		static mat4<T> rotation( const T& alpha, const T& beta, const T& gamma ) {
			const T sina = sin( alpha );
			const T cosa = cos( alpha );
			const T sinb = sin( beta );
			const T cosb = cos( beta );
		...
		static mat4<T> ortho( float left, float right, float bottom, float top, float near, float far ) {
